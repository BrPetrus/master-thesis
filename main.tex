...
curvilinear structure segmentation \parencite{Mou2020}, and improvements to
U-Net networks \parencite{Rundo2019}.
...
\parencite{Ceran2022TNTdetect}. In this paper the authors talk about how to deal
with the difficulty of hand marking the tunnels. They talk about how they used
four human experts to correctly identify TNTs (as individual experts' label
varied), and what was done to make the labels as clear as possible. Then they
divised a two step procedure, in the first a classification network works on
tiles to identifies where a TNT possible is, and then a second network is used
to finely segment the TNTs pixel wise. Unfortunately they do not go into details
of their networks. I tried to contact one of the authors to get the code and
data, but they did not respond. In the paper they talk about using a U-Net like
architecture for the segmentation part, formed of ResNet blocks. They also used
a pretrained encoder. The architecture is called AuraNet [TODO cite].
...
\parencite{kong2018roadsensing} \parencite{Ceran2022TNTdetect}. Our TNTs mostly
...
% TODO make this work
The CS-Net paper's authors tried to improve the performance of UNet architecture
on curvilinear structures, which are generally thin and curvy structures found
in many places in biology like blood vessels in images of retina. The authors
noted that many neural networks have trouble connecting faint blood vessels
leading to connectivity issues. Their solution was to introduce so called CSAM
into the standard U-Net like network.
...
After downsampling the data, the upsampling is introduced to revert the image to
...
The main reasons for choosing this architecture compared to other are that
the U-Net architecure is well estabilished, widely used, and achieves very good
performance. Crucially, it has a usefull property that it does not need a vast
dataset to train properly. This is demonstrated in \parencite{Ronneberger2015},
where they trained the network with just 30 images.
...
the depth cannot be changes, and that the information about the whole data is
very quickly compressed into a single dimensions, potentially inhibiting the
...
more details. We are going to defined 3D and 2D version of the network, by which
...
information. This lead the author to use anisotropic kernels 
...
called the squeeze and excitation blockm, can improve the performance across
various datasets and tasks. 

The general principle of a squeeze and excitation block is illustrated in Figure
\ref{fig:seblock}. The main idea is that we let the network learn
...
The researchers postulate that this improves the network's quality of
representation by modeling the interpedencies between channels of the output of
the convolution layers \parencite{Hu2018}.

The squeeze and excitation (SE) block, shown in Figure \ref{fig:seblock}, consists of
four operations: truncate, squeeze, excitation, and scaling. The truncate
operation shown as $F_{TR}$ is a general convolutional operator, which reduces the
dimension. This step is not crucial in the scheme, but it is mainly to
illustrate the point, that SE block is usually built upon some operation. More
importantnly, the transdormed data is taken and run through a squeeze operation,
which performs some global operation - our case global average pooling - in the
...
--- and they wanted it to be non-mutually exclusive between channels. These
properties are fullfilled by applying two convolutions followed by ReLU and
sigmoid respectively. Note that the first convolution produces output with less
than the original number of channels and the ratio is called the reduction
factor. Finally the last convolution expands the number of channels to the
original size. The comvination of squeeze and excitation allows the network to
learn how to weight each channel (the scale operation mentioned above) by
...
As mentioned in the previous Chapter [TODO cite], we have just two fully
annotated time frames of 7x512x512 voxel size. Splitting such a small dataset
into training and testing sets is difficult and is further complicated by the
fact that we want to experiment with using fully 3D convolutional networks.
Since we do not want to mix testing and training data together, if we were to
cut the data accross the depth axis, we would introduce some bias, as the pillar
are nearly horizontal and the tunnels can go through multiple of this slices.
Moreover, if we were to always take all seven depth slices this would lead to
basically using one time frame for training and the other for testing, and using
a single image is in our opinion unfeasible.

However, this is not a lost cause, as UNets have demonstrated that compared to
some other architectures they do not need as much data to train. In the original
paper just a few dozen images were used to train the networks to segment medical
images \parencite{Ronneberger2015}. Similarly we were able to reproduce the
results of the CS-Net paper \parencite{Mou2021} and train a retina vessel
segmentation network with just a few dozen images.

To find the few dozen images we decided to take a different approach. While
splitting the dataset into quadrants would still yield only about 8 images, so
instead we decided to look at the level of the annotated instanced of tunnels.
Since there are more than 100 of these tunnels, we can train a network on just
cut outs of the tunnels. Then we can tile the whole images and stitch the
predictions of the neural networks similarly as was done in
\parencite{Ronneberger2015}.

The actual splitting logic iterates through every single tunnel and cuts it out.
As the tunnels have different sizes we decided to cut out the bounding boxes of
the tunnels with a set of minimum size. For the minimum size we decided to go
for a 7x90x90 shape as that confortably fits majority of tunnels and provides a
decent context around it. Decent number of tunnels are smaller than this. To
...
It is still important to be somehow able to split the dataset into the training
and testing sets. As the dataset is quite limited choosing a random
subset to evaluate the perfomance is not ideal as then there could be a random
chance that some tunnel from testing set will be visible in a cutout of a tunnel
in the training set. Another proble is how the various parts of the dataset do
not have the same number of tunnels, tunnels of the same morphology and the same
amount of noise and irrelevant structures. For example in the upper right corner
in Figure \ref{fig:upperright} we can find some irrelevant structures which are
not present in the rest of the dataset. Given these problems it is quite
difficult to split the dataset in such a way, that all possible types of
artifacts and tunnel structures would be visible in both testing and training
subsets.

We solved this problem by doing a quadrant-based spatial 4-fold
cross-validation. In each training run, one of the four quadrants is chosen to
be the testing quadrant and all cutouts belonging to this quadrant form the
...
Figure \ref{fig:folding-visualisation} where a possible split is shown in two
colours, red stands for cutouts belonging to the testing subset. The decision if
a cutout belongs to the testing subset or not is dictated by looking at the
overlap with the training quadrant. If the cutout lies with more than 20\% or
has more than 50 pixels in the testing quadrant, it will not be included in the
...
some tunnel by the biologist. The BCE loss is defined as:

\[ L_{BCE} = -w \] %TODO: finish
...
to the ideal mask, we get closer to 0. The loss function was defined using as in
the paper \parencite{ma2020segmentationlossodyssey} as:
...
\section{Optimizers, learning rates, regularization}
