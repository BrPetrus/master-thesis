...
\thesissetup{
    ...
    abstract    = {%
        This thesis addresses the automated semantic segmentation of tunnelling nanotubes (TNTs) in 3D \textit{Drosophila} wing microscopy images. The work tackled challenges posed by a high spatial anisotropy of the analysed image data and its sparse reference annotations.

        The primary contribution is the development of AnisoUNet, an anisotropic U-Net-like architecture designed to effectively handle the highly anisotropic nature of the processed 3D image data. The solution utilised a robust 4-fold cross-validation pipeline.

        The study demonstrates the feasibility of using U-Net-like architectures for segmenting nanoscale curvilinear structures. The resulting segmentation capability offers a basis for automating the quantitative analysis of TNT morphogenesis and dynamics.
    },
    thanks      = {%
        I am very grateful to my advisor, Assoc. Prof. Martin Maška, Ph.D., for his sustained mentorship, constructive critiques, and ready availability throughout the whole time.
...
Semantic segmentation is a critical task in computer vision that involves assigning a specific class label to every image element in a given image. This thesis focuses on the segmentation of tunnelling nanotubes (TNTs), which are cell membrane protrusions observed during cell morphogenesis in \textit{Drosophila} wings. While the exact function of these structures remains under investigation, their image-based analysis is currently limited by the difficulty of manually annotating them in three-dimensional space.

The dataset for this research, provided by Prof. Osamu Shimmi from the University of Tartu, captures a cellular structure known as the Interplanar Amida Network (IPAN). This network consists of vertical "pillars" connected by horizontal TNTs, captured using fluorescence microscopy. The image data presents significant challenges for automated analysis: it is heavily anisotropic, and while the dataset spans twenty frames, reference annotations were provided for only two time frames. As such, the dataset is sparsely annotated.
...
\chapter{Image Data / Biological Background}
\label{sec:data}
% todo: probably add some more examples
During cell morphogenesis, growth of cell membrane protrusions can been observed using fluorescence microscopy. Their effect on the growth is still not fully understood, but has sparked multiple theories. Some researchers speculate they could be used in long-range signalling or are involved in the transportation of organelles \parencite{Tran2024Programmed}. 

Our collaborators from the Institute of Molecular and Cell Biology at the University of Tartu in Estonia were involved in studying one of such places where these cell membrane protrusions are present --- the \textit{Drosophila} wings --- and the findings are available in \parencite{Tran2024Programmed}. They have used in vivo imaging to study the cellular structures.
\begin{figure}[tb]
    \begin{center}
        \includegraphics[width=0.25\textwidth]{resources/data/wings.png}
    \end{center}
    \caption{A schematic of the structure of the \textit{Drosophila} wings. Two nearly identical layers of epithelium cells are supported by microtubule protrusions, which are vertical and referred to as pillars in this work. There are also horizontal protrusions connecting the pillars in nearly horizontal direction --- the so-called tunnelling nanotubes or TNTs. Taken from \parencite{Tran2024Programmed}.}
    \label{fig:wingsdiagram}
\end{figure}
% todo: CITE!

During the development of the larva, specifically in the pre-pupal stage, the pupal wing is formed. Afterwards, the single-layer wing separates into two individual layers of epithelial cells, as shown in Fig. \ref{fig:wingsdiagram}. The nearly microtubule-mediated vertical protrusions connecting the layers will be referred to as pillars, whereas the approximately horizontal protrusions will be called tunnelling nanotubes (TNTs).

In \parencite{Tran2024Programmed}, the authors refer to the overall structure as Interplanar Amida Network (IPAN). They note that IPAN is dynamic and can change significantly in just half an hour, during which the TNTs may connect with different vertical pillars or completely disappear, while some TNTs exhibit stability. While the authors delve into greater depth regarding what exactly happens during the creation and disappearance of these structures, they are limited to a qualitative investigation. The goal of this thesis was to explore U-Net-like models for segmenting tunnelling nanotubes in these 3D images, thereby helping researchers analyse TNT properties in a quantitative fashion. 

Although multiple datasets were acquired, we obtained sparse reference annotations for only one of them; hence, we will focus on this one in this work. In Fig. \ref{fig:dataexamples}, a single z-slice and a volume visualisation of the image data are shown. The whole dataset comprises twenty frames and was captured by using a single fluorescent dye. Each frame consists of seven $512 \times 512$ grayscale, 16-bit images. Each of the z-slices represents a region of $56.32 \times 56.32 \, \mu m$ and the slices are $1 \, \mu m$ apart, meaning that each voxel is about $1 \times 0.11 \times 0.11 \, \mu m$ in size. Since the dye is activated in cell membranes, the pillars in our data appear hollow. 

\section*{Labelling and preprocessing}

The biologists created a protocol which they followed by annotating the samples. They decided to preprocess the data by converting the 16-bit grayscale images into 8-bit grayscale images using a 1\% percentile stretch operation.

After preprocessing is done, the protocol states that, by picking random frames, at least 100 instances of tunnels must be annotated, and each frame must be fully annotated. Also, segments originating from the surrounding tissue should be ignored. According to this protocol, two frames were picked — the 0th and 17th frames — to label. The reference annotations for a single slice of the 0th frame are shown in Fig. \ref{fig:gt-slice-3}. Overall, 127 tunnels are fully annotated.

Sometimes it is rather challenging to decide if the protrusion is a TNT or not. In Fig. \ref{fig:gt-slice-3}, two protrusion-like structures are highlighted using yellow circles. Although their structure appears essentially the same as that of a TNT, it is unclear why they were not labelled too. This illustrates a possible difficulty in assessing the segmentation accuracy.

Afterwards, the labels were manually checked, and two cases of label duplication were identified and corrected by assigning a unique number to the new connected component.

\begin{figure}[p]
    \centering
    \begin{overpic}[width=0.42\textwidth]{resources/t017z0_saturated1percent.png}
        \centering
        \put(3,90){\Large  \textcolor{white}{\textbf{(a)}}}
    \end{overpic}\hfill
    \begin{overpic}[width=0.55\textwidth]{resources/data/volume_viewer_modified.png}
        \put(3,68){\Large{\textbf{(b)}}}
    \end{overpic}
    \caption{(a) Single z-slice of the image data at T=17. (b) Volume visualisation of the IPAN structure. Both images were contrast-enhanced for visualisation purposes.}
    % \caption{A single z-slice of the image data at the 17th frame is shown. Right: a volume visualisation of the IPAN strudata from a different frame. Both images were contrast-enhanced for visualisation purposes.}
    \label{fig:dataexamples}
\end{figure}

\begin{figure}[p]
    \centering
    \begin{overpic}[width=0.48\textwidth]{resources/mask000-slice3-composite.png}
        \centering
        \put(3,90){\Large  \textcolor{white}{\textbf{(a)}}}
    \end{overpic}
    \begin{overpic}[width=0.48\textwidth]{resources/mask000-slice3-saturated.png}
        \centering
        \put(3,90){\Large  \textcolor{white}{\textbf{(b)}}}
    \end{overpic}\hfill
    \caption{(a) Single z-slice of the image data (contrast-enhanced) with overlaid tunnel labels in magenta. Two yellow circles emphasise two places with potentially missing labels. (b) The instance labels for that particular z-slice.}
...
We began by researching papers that explicitly dealt with TNTs. One that used a non-machine learning approach was found \parencite{Hodneland2006Automated}, and another that employed a machine-learning approach with a U-Net architecture was also found \parencite{Ceran2022TNTdetect}. Later, we examined solutions related to the topic of curvilinear structure segmentation \parencite{Mou2021}, and finally explored generic improvements utilising an attention mechanism in \parencite{Rundo2019}.

The afore-mentioned U-Net architecture was first described in a seminal paper by Ronneberger et al. \parencite{Ronneberger2015} in 2015. The authors describe the architecture and use it for the semantic segmentation of biomedical images. The authors achieved impressive results with a minimal dataset of about 30 images. Since then, the architecture has become a standard method for solving segmentation tasks in various modalities; thus, we have also adopted this approach. A more detailed explanation of the architecture can be found in Section \ref{sec:unet}.

% todo: check if they are really looking at the same thing as us

The first paper that explicitly mentions the segmentation of TNTs is by Hodneland et al. \parencite{Hodneland2006Automated}, who were also examining protrusions connecting cells. However, in their case, the tunnels form long, straight lines, unlike our TNTs, which are not always straight. The authors describe a multistep algorithm that employs two channels — one for staining cell membranes and TNTs, and the other for staining cytoplasmic compartments — to segment the TNTs. Since our data was captured in a single channel only, this approach cannot be used.

The other paper, which mentions explicitly tunnelling nanotubes, is \parencite{Ceran2022TNTdetect}. In this paper, the authors developed a tool for segmenting TNTs called TNT.AI. It involves a two-step procedure. In the first step, a classification network works on tiles to identify where a TNT is likely present, and then a second network is used to finely segment the TNTs pixel-wise. Unfortunately, they do not go into details about their networks. I tried to contact one of the authors to get the code and image data, but they did not respond. In the paper, they discuss using a U-Net-like architecture for the segmentation part, which is composed of ResNet blocks. 

As neither of these papers provided working code examples or was applicable to our image data, we decided to look at the tunnels as an example of curvilinear structure. Bibiloni et al. \parencite{bibiloni2016curvilinear} provide a complete definition of curvilinear structures based on their geometric and photometric characteristics. However, they also provide a simpler, rough definition, which is enough for our purposes. They can be defined as thin, long, line-like regions with different intensities than their neighbouring pixels \parencite{bibiloni2016curvilinear}. Some examples of curvilinear structures include river finding, blood vessel segmentation, or lung airways segmentation \parencite{kong2018roadsensing, Mou2021}. Our TNTs mostly fit this description as their shape is thinner compared to the vertical protrusion, and they are clearly different in intensity from the background.

Mou et al. \parencite{Mou2021} use a modified U-Net to segment curvilinear structures such as blood vessels in the retina. They investigate the use of a typical U-Net and introduce the so-called spatial and channel attention module (CSAM), which is purported to help maintain the connection of faint curvilinear structures.
...
Chapter \ref{sec:research} introduces the U-Net architecture in conjunction with its uses in related works. The main idea was as follows: in other tasks, such as image classification, we typically have an encoder followed by a neck, which culminates in a classification head. The authors speculated on how to extend this mechanism to achieve pixel-wise classification, or in other words, semantic segmentation. They propose placing several upsampling layers after the encoder part to upsample the extracted features \parencite{Ronneberger2015}.

The overall diagram is shown in Fig. \ref{fig:unetdiagram}. All convolutional layers use a $3 \times 3$ kernel followed by a Batch normalisation and ReLU operation, while downsampling and upsampling operations use $2 \times 2$ kernels.
...
The presented model works with 2D data, but our data is 3D. Fortunately, the modification is not difficult; all convolutions can be replaced with their 3D equivalent. Moreover, we wanted to obtain an output size equal to the input data; therefore, padding was used in the convolutions. Downsample operations were implemented using a 3D max-pool operation with a $2 \times 2 \times 2$ kernel, and upsampling was technically implemented with transposed convolutions using a $2 \times 2 \times 2$ kernel shape. Finally, as our task is 2-class semantic segmentation, our output has a single channel encoding both classes, as 1.0 corresponds to a tunnel and 0.0 corresponds to anything else. Due to the nature of halving all spatial dimensions isotropically in the downsampling layers, after just two downsampling steps, the z dimensions are reduced from seven to one; thus, our model has two downsampling layers. This implementation is referred to as BasicUNet, which is our first model.
...
Since our data is heavily anisotropic, we are limited in how much we can downsample along the z-axis. As we have only seven z-slices, we can downsample only twice. This presents an issue, as we cannot experiment with a deeper network. Moreover, after just two downsampling steps, the depth of extracted features is reduced to a single slice, which we believe means that the network has to quickly compress all relevant information, potentially inhibiting the expressive performance of the network.
...
There is still a question of what kernel size to use for the horizontal layers. In Section \ref{sec:diff-architectures} we study this aspect in more detail. We will define the 3D and 2D versions of the network, by which we mean that 3D networks use $3 \times 3 \times 3$ kernel shapes in their convolutional layers, while 2D networks use $1 \times 3 \times 3$ kernel shapes.

The idea to test the difference in their performance came from the following two concepts: when a human is looking at the images, they can roughly estimate what are tunnels and what are pillars based on the overall shape, but it is dramatically easier if they are allowed to see multiple z-slices; hence, the 3D version with their greater receptive fields reaching other z-slices, would benefit in terms of their accuracy. On the other hand, networks with 3D kernels have more trainable parameters; thus, they might be harder to train, and as we have a sparsely annotated dataset, this could become a hurdle, causing the 2D version to perform better.

Overall, such a network with anisotropic pooling operations is referred to as AnisoUNet throughout the remainder of this thesis.

\section{The Channel and Spatial Attention Module}
\label{sec-csammodule}

This section discusses the channel and spatial attention module (hereafter referred to as CSAM), as mentioned in \parencite{Mou2021}. 
...
\begin{figure}[htbp]
    ...
    \end{center}
    \caption{A squeeze-and-excitation block. The input data $X$ is processed to create $\tilde{X}$. First, the data is truncated by a $F_{tr}$ operation to form features $U$. The rescale operation $F_{scale}$ rescales the channels according to their importance learnt in the excitation ($F_{ex}$, typically a convolutional layer) and squeeze operations ($F _ {sq} $, typically a global average pooling). Taken from \parencite{Hu2018}}
    \label{fig:seblock}
\end{figure}
...
While the authors in \parencite{Hu2018} talk about placing this object after the truncating operation, and give concrete examples of where to place it in Inception and ResNet blocks, the U-Net architecture is not mentioned. We briefly experimented with placing a single SE block in the bottleneck part of the U-Net, similarly to how the CSAM was used; however, this was later replaced by using the architecture described in \parencite{Rundo2019}. In their work, they suggested a different placement of the SE blocks inside a U-Net-like architecture. It is called USE-Net, and the placement is visible in Fig. \ref{fig:usenet}. The authors place an SE block before the concatenation and at every step of the upsampling. 

This is the last architecture we will explore in this work and will be referred to as the AnisoUNet-UseNet variant. 

\chapter{Training}
\label{sec:training}
This chapter explains how the aforementioned neural networks were trained, how the image data was split to allow cross-fold learning, and finally, how the network can be run to segment images of different sizes.

\section{Data Splitting}
\label{sec:processing_data}
% [TODO mention alternatives to supervised learning somewhere]

As mentioned in Chapter \ref{sec:data}, we have only two fully annotated frames, each of $7 \times 512 \times 512$ voxels. Splitting such a small dataset into training and testing sets presents a challenge, which is further complicated by the need to experiment with fully 3D networks. If we were to treat each depth slice independently, it would allow mixing of testing and training data; however, this could introduce bias. This bias arises because the pillars are nearly vertical, meaning their position is roughly the same in all z-slices, and the tunnels are not limited to a single z-slice. Moreover, if we consistently use all seven depth slices, we would essentially be training on one time frame while testing on a different one. In our opinion, this approach is unfeasible.

However, as U-Nets have demonstrated, they do not require a vast labelled dataset to be trained. In the original paper \parencite{Ronneberger2015}, a few dozen images were used to train the networks to segment medical images. Similarly, we were able to reproduce the results of the CS-Net paper \parencite{Mou2021} and train a retina vessel segmentation network with just a few dozen images.

While splitting the dataset into quadrants would still yield only about eight images, we decided to look at the level of the annotated instances of tunnels. Since there are more than 100 of TNTs, we can train a network on just the crops of the TNTs. Then we can tile the entire image and stitch the predictions of the neural networks similarly to what was done in the original U-Net paper \parencite{Ronneberger2015}.
...
This is still quite challenging to do correctly, as not every tunnel lies neatly inside one of the quadrants. A split used in training is visualised in Figure \ref{fig:folding-visualisation}, where a possible split is shown in two colours: red represents crops belonging to the testing subset. The decision of whether a crop belongs to the testing subset or not is determined by examining the overlap with the training quadrant. If the crop lies with more than 20\% or has more than 50 voxels in the testing quadrant, it is not included in the training subset. If at least 80\% of the crop is inside the testing quadrant, it is considered to be part of the testing subset.
...
Since we can view finding tunnelling nanotubes as a binary classification problem applied at every voxel, it naturally comes to using a binary cross-entropy loss to train the network to segment the images. However, our dataset is heavily imbalanced, as it contains a significantly higher number of negative examples. To somewhat help the training pay more attention to the tunnels, the cross-entropy was weighted by a factor of 50, which was estimated as the ratio of voxels classified as background and voxels assigned to some tunnel by the biologist. The weighted BCE loss is defined as in \parencite{Jadon2020loss,PyTorchBCE}, where $\hat{y}$ stands for prediction, $y$ is the true label, and $\beta$ is the weight:

\[ L_{BCE}(y, \hat{y}) = -(\beta y \log(\hat{y}) + (1-y)\log(1-\hat{y})) \, .\]

To further improve the performance of the training, we drew inspiration from the CS-Net paper \parencite{Mou2021}, which uses Dice loss. Some papers claim that the combination of Dice loss and cross-entropy loss leads to better performance in many segmentation tasks \parencite{ma2020segmentationlossodyssey}. This is defined as taking $L_{Dice} = 1 - \text{Dice}$, meaning that as we obtain segmentation results closer to the ideal mask, we approach 0. The loss function was defined as \parencite{ma2020segmentationlossodyssey}:
\[ L_{Dice}=1-\frac{2\sum_{i=1}^{N}{g_i \cdot s_i}}{\sum_{i=1}^{N}{g_i^2}+\sum_{i=1}^{N}{s_i^2}} \]
...
During the development of the training loop shown in Algorithm \ref{alg:training}, we observed that setting the learning rate to 0.0001 and weight decay to 0.0001 provided a stable learning. As such, we maintained a constant learning rate during training and did not experiment with these values any further. We chose to use an adaptive gradient algorithm for training the neural networks called AdamW, which is a variant of the commonly used Adam optimiser \parencite{loshchilov2019}.

Regularisation is a common technique in machine learning that reduces overfitting by penalising large weights \parencite{Bishop2024DeepLearning}. One of the standard regularisation techniques in deep learning is weight decay and L2 normalisations, which are equal for standard stochastic gradient descent. However, this is not true for the Adam optimiser. In \parencite{loshchilov2019}, the authors argue that using their version, called AdamW, achieves better generalisation performance as it properly decouples the weight decay from optimisation steps. Due to the sparse dataset, the AdamW optimiser with weight decay as a regularisation technique was utilised to train the networks. However, no significant difference in the training performance was observed compared to the standard Adam optimiser.
...
A potential issue with using small tiles centred at tunnels is that we may introduce unwanted bias into the network, as it might learn to always try to find the tunnel in the middle. The design behind using a random spatial crop was to try to suppress this issue, as the program now randomly selects a crop of the desired size, which reduces this problem.

\section{Inference on large images}
\label{sec:runningthenetwork}

All our networks are trained on square crops of size $7 \times 64 \times 64$ voxels, but the original images are larger at $7 \times 512 \times 512$ voxels. This is solved by using an overlap-tiling strategy similar to the one by Ronneberger et al. \parencite{Ronneberger2015}. The original images are split into tiles first, with a configurable amount of overlap. The network sequentially processes the tiles and then stitches them together to create the final prediction. The predictions are then thresholded with a value of 0.5.

\begin{figure}[tbp]
    \centering
    \begin{overpic}[width=0.46\textwidth]{resources/overlaps_and_processing/quad3.png}
        \centering
        \put(3,90){\Large  \textcolor{white}{\textbf{(a)}}}
    \end{overpic}
    \begin{overpic}[width=0.46\textwidth]{resources/overlaps_and_processing/quad3_10px_postprocessed.png}
        \centering
        \put(3,90){\Large  \textcolor{white}{\textbf{(b)}}}
    \end{overpic}

    \begin{overpic}[width=0.46\textwidth]{resources/overlaps_and_processing/quad3_0px_threshold_0_circles.png}
        \centering
        \put(3,90){\Large  \textcolor{white}{\textbf{(c)}}}
    \end{overpic}
    \begin{overpic}[width=0.46\textwidth]{resources/overlaps_and_processing/quad3_10px_threshold_0.png}
        \centering
        \put(3,90){\Large  \textcolor{white}{\textbf{(d)}}}
    \end{overpic}

    \caption{(a) A crop of the dataset which has been contrast-adjusted. (b) Neural network prediction with 10 pixel overlap between tiles and postprocessing applied.(c) Thresholded prediction without any tiling. Possible edge artefacts are emphasised with blue circles. (d) Thresholded prediction with 10 pixel overlap.}

    \label{fig:stitchingoverlap}
\end{figure}

The idea of using a configurable overlap size originated from the following observation. In Fig. \ref{fig:stitchingoverlap}(c), several sharp artefacts can be seen at tile borders, possibly because only part of the structure is visible, leading the network to classify it as a tunnel. This led us to the idea that if we use larger overlaps and aggregate them, these effects on tile borders will get suppressed. As can be seen in Fig. \ref{fig:stitchingoverlap}(d), using a larger overlap suppresses some of these artefacts. For aggregating the overlaps, a simple averaging was chosen due to its simplicity.
...
The value of 100 voxels was chosen, realising that some of the tiny tunnels might get removed, but as will be shown later, the network tends to find more tunnels than what is available in the reference annotations. Hence, I believe this to be a reasonable assumption to make. Figure \ref{fig:histogramoflabelsizes} shows a histogram of the distribution of labels' volumes. Removing components with a volume smaller than 100 voxels removes just a small number of all reference tunnels. In Fig. \ref{fig:stitchingoverlap}(b), the effect of postprocessing on cleaning up artefacts is shown.
...
The high-level logic of the training loop is discussed in Section \ref{sec:optimisers}, and the inference is discussed in Section \ref{sec:runningthenetwork}. This chapter addresses some of the technical details of the implementation. The complete code, written in Python and Bash, is available in the thesis's archive along with several trained networks and a full user's reference.
...
\begin{itemize}
    \item{\texttt{MLflow} \parencite{Zaharia2018MLflow} is a tool for handling machine learning pipelines. It allows for tracking experiments, comparing results, saving and loading models, deploying models, and more. In this work, it's nice to use UI was used to keep track of the experiments.}
    \item{\texttt{NumPy} \parencite{Harris2020Numpy} has become one of the fundamental packages in scientific computing in the Python ecosystem. It provides a very efficient implementation of many common operations on N-dimensional arrays.}
    \item{\texttt{scikit-learn} \parencite{Pedregosa2011scikit} provides a great framework for building machine learning models, not just deep learning models. The library was primarily used for splitting datasets and utilising its framework for handling batching. }
    \item{\texttt{scikit-image} \parencite{vanderWalt2014skimage} is a package providing various algorithms for processing images, e.g. segmentation algorithms, point transform, and mathematical morphology. I used it to handle the postprocessing of masks and to find connected components.}
    \item{\texttt{PyTorch} \parencite{Ansel2024PyTorch} is a popular deep learning framework with a focus on flexibility and speed. This enables users to create highly performing neural networks quickly. It automatically creates computational graphs, allowing it to compute gradient updates efficiently. It automatically handles connecting to various hardware, such as CPUs and GPUs. These advantages made \texttt{PyTorch} one of the most used packages in the Python machine learning ecosystem.}
    \item{\texttt{MONAI} \parencite{Cardoso2022Monai}is another open-source framework for building neural networks on top of \texttt{PyTorch}. It focuses on building models used in the medical area. In the project, this package handles augmentations of the dataset, providing first-class support for 3D data, unlike \texttt{PyTorch}.}
    \item{\texttt{Matplotlib} \parencite{Hunter2007Matplotlib} is a library for visualising data in an interactive fashion. The vast majority of graphs in this text were created using it.}
    \item{\texttt{Pandas} \parencite{Pandas} is a popular library for handling data. It provides a DataFrame object for efficient data manipulation, reshaping, and indexing, among many other things. In this work, the final evaluations were created using the Pandas library.}
\end{itemize}
Along with these tools, a formatter called \texttt{ruff}, a \texttt{tifffile} package for reading and writing images, and \texttt{torchinfo} package for benchmarking the number of parameters of any model were also used.
...
For convenience, I am also including a \texttt{requirements.txt} file, which was automatically generated from a virtual Python environment; however, the officially supported version uses the \texttt {uv} package manager. The details of recreating the environment are described in the file \texttt{README.md}. All of this is available as part of the digital archive in the Information System.
...
\chapter{Evaluation}
This chapter discusses how the different networks were evaluated on the entire dataset and presents the results of an ablation study designed to choose the values of the depth and overlap hyperparameters. The chapter ends with performance benchmarks across the architectures.

\section{Metrics}
% todo : consider citation
For evaluating the resulting predictions, we decided to use Dice scores as they are commonly used in many biomedical papers, but the end user is not interested in the quality of the scores on crops around tunnels, or how every neural network was trained; it is crucial to know how the neural network performs on the whole image.
...
We define a mapping between the predictions and the tunnels in the reference annotation to investigate performance at the tunnel level and the performance on clearly identified tunnels.
...
\begin{figure}[htbp]
    ...
    \end{subfigure}
    \caption{Three vertical pillars, shown as black circles, are connected by TNTs, shown as black regions between black circles. (a) The network found two tunnels. Identifying a real tunnel correctly is considered a true positive (TP) and is shown in blue, while the second prediction, in red, is a false positive. (b) The network predicts a single tunnel $p_1$, which covers both real tunnels $t_1$ and $t_2$. This is considered to be a TP.}
    \label{fig:matchingdiagram}
\end{figure}

In Fig. \ref{fig:matchingdiagram}, we can see a schematic of a situation with three pillars and several predictions. In Fig. \ref{fig:matchingdiagram}(a) a network predicted a single true positive (TP), false positive (FP), and false negative (FN).
In Fig. \ref{fig:matchingdiagram}(b), a situation where a single prediction covers both tunnels is more complicated. This might be present near a cluster of pillars and tunnels, where a clear separation is difficult to see. The tunnels might theoretically even cross each other. While discarding these results is possible, we will consider such a situation a success and label it as finding two TPs.
...
\begin{align*}
 \text{mapping}(P,L)= \{(l,\hat{p})|(\forall l \in L, \hat{p} = \text{argmax}_{p \in P} \text{recall}(p,l)) \\
 \land \, \text{recall}(\hat{p},l) > 0.5 \} \, .
\end{align*}

The decisive factor is how much of the label is covered by the prediction, which is equivalent to the recall score, and we set it to 0.5, as more than half of the label must be covered. This definition formalises the mapping shown in Fig. \ref{fig:matchingdiagram}.
...
The defined matching enables us to quantify how well the network performs at the voxel level, as well as in terms of identifying individual instances of tunnels.
...
\begin{itemize}
    \item{Test(voxel level): Look at the performance on crops of tunnels.}
    \item{Evaluation (voxel level): Look at the performance derived from the overall stitched binary images.}
   \item{Postprocessed overall (voxel level): Look at the performance derived from the overall stitched binary images after postprocessing.}
\end{itemize}

Next, we will use the matches defined above. In these cases, the matches are taken, and a binary image is created by aggregating the binary sets using a binary logical OR operator. This means that for each voxel, if any of the corresponding voxels in the matches are true, the voxel in the resulting binary image will also be true. This aggregation process enables us to create a single comprehensive binary image that captures all the features represented by the matches, effectively highlighting the areas of interest identified earlier, ignoring artefacts caused by false positives and false negatives.

\begin{itemize}
    \item{Postprocessed matches (voxel level): Look at the performance derived from all the matches.}
    \item{Postprocessed clean matches (voxel level): Look at the performance of just the clean matches.}
    \item{Tunnel metric (tunnel level): Here the performance is calculated from the tunnels instead of voxels.}
\end{itemize}
...
It is not apparent how to set the tiling overlap, but the choice of this hyperparameter affects every neural network we have discussed. Also, every advanced neural network built on top of the AnisoUNet network, described in Section \ref{sec:anisounet}, has a configurable model depth.

% TODO: think about citing something
In general, it is accepted that the ability to teach deeper networks has helped improve the performance of neural networks, and, in some sense, the ability to teach deeper networks has led to the neural network revolution. Nevertheless, the size of the network should be connected to the complexity of the problem, both in terms of the size of the context and the image complexity. To determine a reasonable value for these parameters, we conducted a 2-D hyperparameter search study. 

% todo: talk about this in the discussion

Taking the aforementioned AnisoUNet3D architecture, we considered five different depths. The shallowest one has exactly two downsampling layers, so it is referenced as a network with depth equal to two, and the deepest one has six downsampling layers. Six downsampling layers were chosen, as this is sufficient to reduce the original size of 7x64x64 to 7x1x1; therefore, a deeper network is not feasible.

For the overlap hyperparameter, we considered the values 0px, 10px, 20px, 30px, and 40px. The value 40px was chosen arbitrarily, as in this case, the neighbouring cells already share more than half of the volume.
...
In Fig. \ref{fig:evaldepth}, the effect of the depth is studied further on the Evaluation metric. Figure \ref{fig:postprocessoveralldepth} shows the Postprocessed overall metric.
...
Taking a look at Fig. \ref{fig:evalquadsdepth} here, we can see the same metrics, but this time it is further divided among the various quadrants on which the network was tested. What is immediately evident to us is that the error bars are less varied, due to the performance differences across quadrants. Interestingly, in some cases, such as the models with six downsampling layers for the first quadrant, there are substantial performance differences, but no apparent reason for this was found. Since, in this case, all bars represent averages of three numbers, it is plausible that, due to bad luck, the network may not achieve as good a performance as in other runs. Another possible reason is that the deeper variants have significantly more parameters and are overfitting on the sparse dataset.
% TODO talk in discussion

In general, all models performed the best at the first quadrant (top left) and the third quadrant (bottom left), while the performance was lower on the second quadrant (top right) and the fourth quadrant (bottom right). Looking at the means and error bars now, it seems that the model with three downsampling layers performs best or close to best in all quadrants, whereas having more layers can sometimes reach similar performance(like in case of three downsampling steps in the third quadrant) it might also lead to worse; hence, we cannot say that for our input size, the increased depth helps dramatically. However, our previous decision to choose the network of depth three in the following studies still stands, as although the performance difference is not substantial, this network consistently outperforms the smaller network, and at the same time, has many fewer parameters than the larger networks.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.93\textwidth]{resources/evaluations/eval_dice_mean_by_quad_arch.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.93\textwidth]{resources/evaluations/postprocess_overall_dice_by_quad_arch.png}
...
We decided to study the following architectures: a basic implementation of UNet called BasicUNet as described in Section \ref{sec:unet}. The anisotropic version AnisoUNet as described in Section \ref{sec:anisounet}, a version of AnisoUNet with the CSAM module as described in Section \ref{sec-csammodule}, and finally the UseNet variant of AnisoUNet architecture from Section \ref{sec-seblock}. All models use a 10-voxel overlap, and their depth is equal to three downsampling blocks, with the exception of BasicUNet, which has two downsampling blocks. For every AnisoUNet variant, both 3D and 2D versions were trained, as is described in Section \ref{sec:anisounet}.

\subsection*{Metrics}

Moving on to the results, Figs. \ref{fig:eval-diff-arch-dice}- \ref{fig:eval-diff-arch-tunnel} show metrics aggregated across all the cross folds, while \ref{fig:eval-diff-arch-dice-quads}-\ref{fig:eval-diff-arch-tunnel-quads} show the performance for each quadrant. The means and standard deviations of all metrics were calculated exactly the same as in the previous section; again, every combination was trained three times.

We can again see large error bars in all figures, especially when looking at the tunnel metrics in Fig. \ref{fig:eval-diff-arch-tunnel}. Like in the previous section, this is caused by the difference in performance across the folds and will be discussed after analysing the averages across folds.
...
While it is difficult to assess the overall trends due to the error bars, the various attention mechanisms do not provide much improvement for the AnisoUNet neural network. The average values of the evaluation metric for the CSAM and USENet variants are slightly higher for the 3D versions, whereas it is less clear in the 2D case. Moreover, the 2D version of AnisoUNet slightly outperforms the 3D version. The same is happening for the AnisoUNet-CSAM variant.

Moving to the advanced metric, we can observe that postprocessing is slightly improving the Dice score, indicating that the removal of all small components helps eliminate artefacts. While comparing Figs. \ref{fig:eval-diff-arch-clean} and \ref{fig:eval-diff-arch-all}, we can see that the networks reach higher performance when looking at the correctly identified tunnels. This means that the tunnel's accuracy is hindered by finding false tunnels and/or not fully segmenting real tunnels. It is also clear that the difference in performance between the cleanly matched and all matches is not substantial.
...
Moving to the evaluations per quadrant, in Figure \ref{fig:eval-diff-arch-dice-quads}, we can see that the majority of neural architectures perform the best on the first and third quadrant, whereas the second quadrant is more problematic. There does not seem to be a clear winner between the architectures, as a particular attention mechanism outperforms the standard AnisoUNet in one quadrant but is then surpassed in another. Lastly, the difference between the 2D and 3D variants is not dramatic in most quadrants; they differ substantially in the second quadrant, where the 2D versions outperform the 3D ones.

Looking at the advanced metrics, the overall Dice metric in Fig. \ref{fig:eval-diff-arch-dice-overall-quads} shows that, again, the postprocessing consistently improves the performance of all models. Moving to the cleanly matched in Fig. \ref{fig:eval_diff_arch_clean_match_dice_quads} and all matches in Fig. \ref{fig:eval_diff_arch_matched_dice_quads}, it is clear that the networks are achieving substantially better performance compared to the overall score. Again, this suggests that the neural networks have issues with noise and false positives. Interestingly, the noteworthy performance difference between the second quadrant and the rest seems to disappear here. A possible explanation is that while the neural network performs similarly on clear data, its ability to handle noise and differentiate between real tunnels and artefacts varies. Lastly, the tunnel metric in Fig. \ref{fig:eval-diff-arch-tunnel-quads} shows that the networks struggled to correctly identify what is and what is not a tunnel in the second quadrant. The best performance was achieved by the 2D version of AnisoUNet, reaching a Dice score of $0.772$. Again, it appears that the 2D version tends to outperform the 3D version in this task as well. It is also noteworthy that in this metric, the performance of a given network can vary substantially, as in the case of 2D UseNet in the third quadrant. No clear systematic explanation was found, and it is likely caused by random fluctuations in the training performance, as these bars were generated from three values.

\begin{figure}[htbp]
    \begin{center}
        \includegraphics[width=0.8\textwidth]{resources/evaluations/diffarchs/eval_dice_mean_architecture_comparison.png}
    \end{center}
    \caption{Evaluation metrics. Each bar represents the average of twelve model runs across the 4-fold cross-validation. Error bars represent the sample standard deviation.}
...
\begin{figure}[htbp]
    ...
    \end{center}
    \caption{Tunnel Dice metric. Each bar represents the average of twelve model runs across the 4-fold cross-validation. Error bars represent the sample standard deviation.}
...
\begin{table}[htbp]
    ...
    \begin{tabular}{|c|c|c|c|c|c|c|}
        \hline
        \textbf{Architecture} & \textbf{Parameters} & \textbf{peak VRAM [MB]} & \textbf{inference time [ms]} & \textbf{throughput [samples/s]} \\
        \hline
        Aniso(2D) & 7.70 M & 1578.11 & 21.04 & 1521.2 \\ \hline
        Aniso(3D) & 21.71 M & 1689.06 & 35.69 & 896.55 \\ \hline
        UseNet(2D) & 7.73 M & 1578.38 & 22.85 & 1400.62 \\ \hline
        UseNet(3D) & 21.74 M & 1689.32 & 37.55 & 852.1 \\ \hline
        CSAM(2D) & 8.26 M & 1647.08 & 25.40 & 1259.8 \\ \hline
        CSAM(3D) & 22.27 M & 1757.44 & 39.99 & 800.22 \\ \hline
        BasicUNet & 5.42 M & 1449.24 & 18.26 & 1752.58 \\ \hline
    \end{tabular}
    }
    \caption{Memory footprints and inference times for the investigated networks. Each inference time is an average of 200 runs on random data of shape $7\times 64 \times 64$ with batch size of 32. A sample corresponds to a single tile.}
...
\begin{figure}[htbp]
    ...
    \end{subfigure}
    \caption{Predictions on the second quadrant. (a) The original image (contrast-enhanced) with binarised reference annotations in magenta. (b) The prediction of BasicUNet, (c) 3D version of CSAM variant, (d) 2D version of CSAM. Green represents TP, red stands for FP and blue for FN.}
    \label{fig:qualitative-quad2}
\end{figure}

Continuing with the analysis in the third quadrant, where the 2D UseNet performance varied quite substantially. Figure \ref{fig:qualitative-quad3} shows that due to probably bad luck during the training, one of the models (reaching a tunnel Dice score of 0.7) managed to correctly match two of the three tunnels, while the last one failed to match any (tunnel Dice score is about 0.47), due to the poor prediction shown in the blue colour. The poorer prediction on the stitched images lead to missed opportunities with matching.

Finally, we take a look at the first quadrant, which reached the highest scores. In Fig. \ref{fig:qualitative-quad1}, a part of the dataset with minimal artefacts is shown. As can be seen, our networks tend to have a problem with oversegmentation and mark small structures as tunnels even though they are not. While the CSAM and UseNet variants do not improve the prediction by AnisoUNet, the prediction by AnisoUNet looks decent.
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[b]{0.3\textwidth}
        ...
        \begin{overpic}[width=\textwidth]{resources/qualitative/quad3eval/t017-ce-slice7-gimp-crop.png}
            \put(3,85){\Large  \textcolor{white}{\textbf{(a)}}}
        \end{overpic}
        \label{fig:quad3gimp}
    \end{subfigure}
    \begin{subfigure}[b]{0.3\textwidth}
        ...
        \begin{overpic}[width=\textwidth]{resources/qualitative/quad3eval/nice-slice7-crop.png}
            \put(3,85){\Large  \textcolor{white}{\textbf{(b)}}}
        \end{overpic}
        \label{fig:quad3nice}
    \end{subfigure}
    \begin{subfigure}[b]{0.3\textwidth}
        \centering
        \begin{overpic}[width=\textwidth]{resources/qualitative/quad3eval/bad-slice7-crop.png}
            \put(3,85){\Large  \textcolor{white}{\textbf{(c)}}}
        \end{overpic}
        \label{fig:quad3bad}
    \end{subfigure}
    \caption{Predictions on part of the third quadrant. (a) The original image (contrast-enhanced) with reference annotations. Different colours correspond to different tunnels. (b) and (c) Predictions of two different UseNet models. Red means that a tunnel from reference was not matched to any predicted tunnel, while blue means that a predicted tunnel did not match any reference tunnel.}
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}{0.4\textwidth}
        ...
        \begin{overpic}[width=\textwidth]{resources/qualitative/quad1eval/t000-composite-slice-1.png}
            \put(3,87){\Large  \textcolor{white}{\textbf{(a)}}}
        \end{overpic}
        \label{fig:quad1orig}
    \end{subfigure}
    \begin{subfigure}{0.40\textwidth}
        ...
        \begin{overpic}[width=\textwidth]{resources/qualitative/quad1eval/aniso2d-slice1.png}
            \put(3,87){\Large  \textcolor{white}{\textbf{(b)}}}
        \end{overpic}
    ...
    \begin{subfigure}{0.40\textwidth}
        ...
        \begin{overpic}[width=\textwidth]{resources/qualitative/quad1eval/usenet2d-slice1.png}
            \put(3,87){\Large  \textcolor{white}{\textbf{(c)}}}
        \end{overpic}
    ...
    \begin{subfigure}{0.40\textwidth}
        ...
        \begin{overpic}[width=\textwidth]{resources/qualitative/quad1eval/csam2d-slice-1.png}
            \put(3, 87){\Large  \textcolor{white}{\textbf{(d)}}}
        \end{overpic}
        \label{fig:quad1csam}
    \end{subfigure}
    \caption{Predictions on the first quadrant. (a) The original image (contrast-enhanced) with reference annotations in magenta. (b) The prediction of 2D AnisoUNet, (c) 2D version of UseNet variant, (d) 2D version of CSAM variant. Green represents TP, red stands for FP and blue for FN.}
...
The complete source code for the U-Net-like neural network model, which includes the necessary training, evaluation, and inference scripts, is attached as an accompanying digital appendix in a form of a zip file. This archive also features automation scripts to streamline the training and evaluation. To facilitate understanding and setup, the archive includes a comprehensive \texttt{README} file that details the environment configuration, explains the project structure, and provides step-by-step instructions for running the code. Finally, the archive also contains all image data, including the used data splits and the evaluation dataset.
