...
For evaluating the resulting predictions we decided to use Dice and Jaccard
scores as both are very common to use in biomedical papers
\parencite{Ronneberger2015} [TODO] but the end user is not interested in the
quality of the scores on cutouts around tunnels, instead we must somehow use our
neural network for predicting over the whole images.
...
solve this problem by tilling the original images and then stitching them
together. This overlap-tile strategy is used in several papers such as
\parencite{Ronneberger2015} [TODO]. There are roughly two strategies on how to
handle the edges, in works such as \parencite{Ronneberger2015} the neural
network's prediction is smaller than the original size since they do no pad the
intermediate results. Then the sampling of images is done in such a way that
every output pixel has a prediction.

Since, our crop size is rather small, given
the problems with dataset size described in [TODO], we decided to use padding
and always output the same size, and to have a configurable overlap size between
the neighbouring tiles. The idea is better visualised in Figure
\ref{fig:tiling}, where the various overlaps are visualised.
...
The idea to use configurable overlap size came from the following observation.
In Figure \ref{fig:stitchingoverlap0} there can be seen several sharp artifacts
%todo how to cite subfigure?
at places where the tiles were touching, as there were some leftover information
from neighbouring tiles but not quite a good view of the whole object, the
network probably got confused, and mistook the feature for a very short tunnel.
This lead us to the idea that if we use larger overlaps and aggregate them these
effects on boundaries might get fixed. As can be seen in the figure, using
larger overlap and averaging the predictions suppresses a lot of these
artifacts.
...
Even after this step there still might be some leftover artifacts as can be seen
in Figure \ref{fig:stitchingoverlap}, which is not ideal, but these artifacts
are generally, from the way how they arrived small. This leads us to how the
...
some artifacts remain. Moreover, some artifacts actually closely resemble thin
structures but are not a true tunnels. In general to further cleanup this
prediction we decided to use area size.
...
chose the value of 100px, realising that some of the tiny tunnels might get
...
Finnaly, the predictions are thresholded by the value of 0.5. This was chosen as
...
While working on the network the choice of depth and overlap is not obvious. It
is widely accepted that the ability to teach deeper network helped improve
neural networks performance.
% TODO citation needed

In general, the size of the network should be somehow connected to the
...
there is no need to have a deeper network. For reference we also studied the
performance of the BasicUNet version.
...
seven different architectures must be trained to perform this analysis.
Afterwards the seven different models are evaluated for every possible overlap
combination, leading to 30 values. Since neural networks employ randomness to
train, to improve the stability of the results, each architectures are trained
three times. This means that each number in Figure \ref{fig:eval:dice_mean} is
in fact combined from three models, and for this evaluation 21 models were
trained.
...
The results of this ablation study is shown in Figure \ref{fig:eval:dice_mean}
with the highlighted maximum number, which corresponds to the 10 pixel overlap
and a network with three downsampling layers. This was used to fix these
hyperparameters for later studies.
...
and all others is visible in every case.
...
the best numbers, while the networks with two and six downsampling steps are
...
performance differences accross quadrants.
...
size, the increased depth helps dramatically.

Another observation of note is the significantly lower performance of the model
in quadrant 2 when looking at the tunnel metrics like in Figure
\ref{subfig:eval_tunnels}. This explains the large error bars in the overall
performance seen in Figure \ref{fig:eval-depth-tunnels}.
...
TODO: missing some examples or theories why the quad 2 looks bad :(
...
most complex models were created. This came out of the idea, that though that
the network would benefit from looking at not just a single slice, but multiple
slices. Hence, the 3D version of a given model uses (3,3,3) kernel in all
convolutional operations, which are located in the Horizontal blocks, and the 2D
version of a given model uses a (1,3,3) kernel (the first dimension is depth).
...
metrics was calculated exactly the same as in the previous section.
...
Like in the previous section we found that the tunnel metrics are poor in the
second quadrant compared to the other ones. To investigate this behaviour we
investigated closer the output of the 2D version of the AnisoUSENet model, which
is near the top of the score in for every quadrant. Since this metric is
calculated on top of matched predictions and labels, it is important to look at
how well did the matching work in this quadrant. The precision and recall of
matching is visible in Figure \ref{fig:eval-diff-arch-dice-overall-quads}, and
as is clearly seen matching is performing sgnificantly worse on the second
quadrant.

This provides an interesting observation as we can clearly see from the dice
score on postprocessed images in Figure
\ref{fig:eval-diff-arch-dice-overall-quads}, that while the segmentation are not
much worse than in the other folds, the predictions cannot be clearly mapped to
the labels on an instance level; moreover, looking at the
\ref{fig:eval_diff_arch_matched_dice_quads} and Figure
\ref{fig:eval_diff_arch_clean_match_dice_quads} we can also see that the network
poorly segments tunnels which we were able to match, which is not the case for
other quadrants, where we can generaly see an improvement when looking just at
the Dice score of matched tunnels.
...
\begin{figure}
    \begin{center}
        \includegraphics[width=0.9\linewidth]{resources/evaluations/diffarchs/eval_dice_mean_by_quad_arch.png}
    \end{center}
    \caption{}
...
\begin{figure}
    \begin{center}
        \includegraphics[width=0.8\linewidth]{resources/evaluations/diffarchs/postprocess_overall_dice_by_quad_arch.png}
    \end{center}
    \caption{}
...
\begin{figure}
    \begin{center}
        \includegraphics[width=0.8\linewidth]{resources/evaluations/diffarchs/postprocess_clean_matched_dice_by_quad_arch.png}
    \end{center}
    \caption{}
...
\begin{figure}
    \centering
    \includegraphics[width=0.6\linewidth]{resources/evaluations/diffarchs/precision_recall_quads.png}
    \caption{}
    \label{fig:eval-prec-recall-quads}
\end{figure}
