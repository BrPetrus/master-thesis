...
Sometimes it is rather challenging to decide if the protrusion is a TNT or not. In Figure \ref{fig:gt-slice-3}, two protrusion-like structures are highlighted using yellow circles. Although their structure appears essentially the same as that of a TNT, it is unclear why they were not labelled too. This illustrates a possible difficulty in assessing the segmentation accuracy
...
\begin{figure}[p]
    ...
    \begin{subfigure}{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{resources/mask000-slice3-composite-circles.png}
    \end{subfigure}
    \begin{subfigure}{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{resources/mask000-slice3-saturated.png}
    \end{subfigure}\hfill
...
\begin{figure}[p]
    ...
    \end{subfigure}
    \caption{A single z-slice of the image data at time 17 is shown. Right: a volume visualisation of the IPAN strudata from different time slots. Both images were contrast-enhanced for visualisation purposes.}
...
The idea of using a configurable overlap size originated from the following observation. In Fig. \ref{fig:stitchingoverlap0}, several sharp artefacts can be seen at tile borders, possibly because only part of the structure is visible, leading the network to classify it as a tunnel. This led us to the idea that if we use larger overlaps and aggregate them, these effects on tile borders will get suppressed. As can be seen in Fig. \ref{fig:stitchingoverlap10}, using a larger overlap suppresses some of these artefacts. For aggregating the overlaps, a simple averaging was chosen due to its simplicity.
...
\begin{itemize}
    \item{MLflow \parencite{Zaharia2018MLflow} is a tool for handling machine learning pipelines. It allows for tracking experiments, comparing results, saving and loading models, deploying models, and more. In this work, it's nice to use UI was used to keep track of the experiments.}
    \item{NumPy \parencite{Harris2020Numpy} has become one of the fundamental packages in scientific computing in the Python ecosystem. It provides a very efficient implementation of many common operations on N-dimensional arrays.}
    \item{scikit-learn \parencite{Pedregosa2011scikit} provides a great framework for building machine learning models, not just deep learning models. The library was primarily used for splitting datasets and utilising its framework for handling batching. }
    \item{scikit-image \parencite{vanderWalt2014skimage} is a package providing various algorithms for processing images, e.g. segmentation algorithms, point transform, and mathematical morphology. I used it to handle the postprocessing of masks and to find connected components.}
    \item{PyTorch \parencite{Ansel2024PyTorch} is a popular deep learning framework with a focus on flexibility and speed. This enables users to create highly performing neural networks quickly. It automatically creates computational graphs, allowing it to compute gradient updates efficiently. It automatically handles connecting to various hardware, such as CPUs and GPUs. These advantages made PyTorch one of the most used packages in the Python machine learning ecosystem.}
    \item{MONAI \parencite{Cardoso2022Monai}is another open-source framework for building neural networks on top of PyTorch. It focuses on building models used in the medical area. In the project, this package handles augmentations of the dataset, providing first-class support for 3D data, unlike PyTorch.}
    \item{Matplotlib \parencite{Hunter2007Matplotlib} is a library for visualising data in an interactive fashion. The vast majority of graphs in this text were created using it.}
    \item{Pandas \parencite{Pandas} is a popular library for handling data. It provides a DataFrame object for efficient data manipulation, reshaping, and indexing, among many other things. In this work, the final evaluations were created using the Pandas library.}
\end{itemize}
