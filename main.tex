...
fluorescent microscopy. Their effect in the growth is still not fully understood,
...
biology at the University of Tartu in Estonia were inserted in studying one of
...
During the development of the larva, specifically in the pre-pupal stage, the
...
The vertical protrusion connecting the layers will be referred to as the pillars,
while the vertical microtubular protrusion will be called tunneling nanotubes
(TNTs).

The overall structure, referred by them as Interplanar Amida Network (IPAN) is
...
of these structures, they are limited to a qualitative investigation. The goal of
...
The dataset is comprised of 20 time slots [todo check] and was captured by using a
single dye in fluorescent microscopy technique. Each time snapshot consists of
...
applied to further enhance the images. 
...
be fully annotated. Also segments outgoing from the surrounding tissue should be
...
the biologist did not also label the TNTs. This illustrates a possible
difficulty when assessing the segmentation accuracy later on.  % todo: make this
more natural
...
The other paper which specifically mentions tunneling nanotubes is
...
to make the labels as clear as possible. Then they devised a two step procedure,
...
for our data, we decided to look at the tunnels as what is called a curvilinear
...
\parencite{bibiloni2016curvilinear}. Some examples of curvilinear structures
...
I decided to use a combination of Python and Bash to implement the thesis. The
