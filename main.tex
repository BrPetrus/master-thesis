...
transposed convolution layer, which doubles the spatial dimensions at the cost
of halving the number of channels. Each upsampling is followed by concatenating
...
diagram, where the concatenations are shown by the horizontal lines.
...
The Figure \ref{fig:unetdiagram} is visualising how the U-Net architecture can be
used to train a 2D model, but there is not reason why we could not use 3D
versions of convolution, maxpool and transposed convolution and in fact this
...
Overall such network with anisotropic pooling operations will be referred to as
...
a squeeze operation where the channels are compressed and then an excitation
...
network's quality of representation by modeling the interdependencies between
...
some operation. More importantly, the transformed data is taken and run through
...
non-linear dependencies --- and they wanted it to be non-mutually exclusive
between channels. These properties are fulfilled by applying two convolutions
...
channels to the original size. The combination of squeeze and excitation allows
